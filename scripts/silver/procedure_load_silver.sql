...
BEGIN
   DECLARE @start_time DATETIME, @end_time DATETIME, @batch_start_time DATETIME, @batch_end_time DATETIME;
    ...
    PRINT '>> Inserting the data into the table : silver.crm_prd_info'
    ...
    ---------------------------------
    SET @start_time = GETDATE()
    PRINT '>> Truncating the table : silver.crm_sls_info'
    TRUNCATE TABLE silver.crm_sls_info

    PRINT '>> Inserting the data into the table : silver.crm_sls_info'
    ...
    PRINT '>> Insaerting the data into the table : silver.erp_cust_az12'
    ...
    SELECT 
    REPLACE(cid, '-', '') AS cid,
    CASE WHEN TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), '')) 
            IN ('US','USA') THEN 'United states'
	     WHEN TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), '')) 
            = 'DE' THEN 'Germany'
	     WHEN NULLIF(TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), '')),'') 
            IS NULL THEN 'n/a'
 ...
 PRINT 'Batch Loading Duration :' + CAST(DATEDIFF(SECOND, @start_time, @end_time) AS NVARCHAR) + 'seconds'
 PRINT '-----------------------'
 END TRY
 BEGIN CATCH
    PRINT '----------------------------------';
